    ...
    def _prepare_kv_cache(
        ...
        for cache_length in past_key_values_buckets.keys():
            if cache_length >= current_sequence_length:
                ...
                # to the new one. Only copy when target is >= source (never downgrade).
                if current_past_key_values_bucket is not None and cache_length != current_past_key_values_bucket:
                    from_size = past_key_values_buckets[current_past_key_values_bucket].get_max_cache_shape()
                    to_size = past_key_values_buckets[cache_length].get_max_cache_shape()
                    if to_size < from_size:
                        # Target bucket is smaller than source - skip, use a larger bucket
                        continue
                    self._copy_kv_cache(
